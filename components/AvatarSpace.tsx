...
export default function AvatarSpace({
  ...
  // 画面共有の全画面表示(expandedMedia)専用の拡大縮小。プレビュー枠から
  ...
  useEffect(() => {
    const apply = () => {
      ...
      room.remoteParticipants.forEach((participant) => {
        ...
        participant.videoTrackPublications.forEach((pub) => {
          // ScreenShareはここでは対象外とし、「自動視聴」effectが個別に
          // 制御する。
  ...
  const activeSharerId = screenSharing
    ? selfId.current
    : (visibleScreenShares[0]?.id ?? null);
  const activeSharerName = screenSharing
    ? "あなた"
    : (visibleScreenShares[0]?.name ?? "");
  const activeSharerStream = screenSharing
    ? screenStreamRef.current
    : activeSharerId
      ? remoteScreenStreams[activeSharerId]
      : undefined;
  const activeSharerPreviewImage = activeSharerId
    ? screenPreviewImages[activeSharerId]
    : undefined;
              ...
              <div className="absolute right-4 top-4 z-10 flex items-center gap-2">
                {expandedMedia.kind === "screen" && (
                  <>
                    <button
                      onClick={() =>
                        setFullscreenZoom((z) => Math.max(0.5, Math.round((z - 0.1) * 10) / 10))
                      }
                    ...
                    <button
                      onClick={() =>
                        setFullscreenZoom((z) => Math.min(2.5, Math.round((z + 0.1) * 10) / 10))
                      }
                      className="flex h-10 w-10 items-center justify-center rounded-full bg-black/60 text-lg text-white hover:bg-black/80"
                      aria-label="拡大"
                    >
