...
// (announcementsテーブルのRLSは既にそれを見据えた設計)。
...
> = {
  ...
  pro: {
    label: "Pro",
    subLabel:
      "（ミーティングルーム1つ　人数上限：30名／画面共有・ビデオ通話・音声通話：無制限）",
    priceLabel: "9,800円/月",
    priceYen: 9800,
    maxRooms: 1,
    maxPeoplePerRoom: 30,
...
// PC: タブ非アクティブが続いた際、在席ステータスを「離席中」に自動で
// 切り替えるまでの猶予秒数(退室させるDESKTOP_AUTO_LOGOUT_SECONDSとは
// 別の、より短い閾値)。以前は非アクティブになった瞬間に即座に離席中へ
// 切り替えていたが、少し席を外しただけで離席中表示になってしまうのを
// 避けるため、この秒数だけ猶予を持たせるようにした。
export const DESKTOP_AUTO_AWAY_SECONDS = 10 * 60;
...
export const AVATAR_HITBOX_HEIGHT = 15; // 当たり判定の高さ(px)
export const CAR_HITBOX_HEIGHT = 10; // 車表示中(shift+x)の当たり判定の高さ(px)。幅はAVATAR_HITBOX_WIDTHのまま変えない。
