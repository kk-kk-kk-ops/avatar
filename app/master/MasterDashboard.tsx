...
export default function MasterDashboard({
  ...
  return (
    ...
    <div className="flex min-h-screen">
      ...
      <aside
        className={`fixed inset-y-0 right-0 z-50 flex w-56 shrink-0 flex-col overflow-y-auto border-l border-slate-800 bg-slate-900 transition-transform duration-200 md:sticky md:top-0 md:h-screen md:translate-x-0 md:self-start ${
    ...
    {pendingNav && (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
        <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-xl">
          <p className="text-sm font-bold text-slate-800">別のメニュー画面へ移動しますか?</p>
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            >
              {pendingNavSaving ? "保存中..." : "保存して移動"}
            </button>
            <button
              onClick={() => resolvePendingNav(false)}
              disabled={pendingNavSaving}
              className="rounded-lg bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-400 disabled:opacity-60"
            >
              保存せず移動
            </button>
            <button
